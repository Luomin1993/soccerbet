...
#每个样本前面27个数据为基本面数据。最后两个为进球数。
data=np.loadtxt('goal_data.csv',dtype='float',delimiter=',')

#将进球标签转换成独热编码 one-hot
def label_change(goal_reses):
    label_num=len(goal_reses);
    change_arr=np.zeros((label_num,5));
    for i in range(len(goal_reses)):
        change_arr[i] = label_change_one(goal_reses[i]);
    return change_arr;    
        

def label_change_one(goal_res):    
    goal = goal_res[0]-goal_res[1];
    if goal>=2:
...
#定义神经网络的输入输出结点，每个样本X为1*27维，以及输出分类Y结果
INPUT_NODE=27
OUTPUT_NODE=5

#定义两层隐含层的神经网络，一层20个结点，一层10个结点
LAYER1_NODE=20
LAYER2_NODE=10
...
def train(data):
    ...
    data_train_x = data[:250,:27];
    data_train_y = label_change(data[:250,27:29]);
    data_test_x  = data[250:,:27];
    data_test_y  = label_change(data[250:,27:29]);
    ...
    #cross_entropy=tf.nn.softmax_cross_entropy_with_logits(logits=y,labels= tf.to_float(tf.arg_max(y_,1), name='ToFloat'))
