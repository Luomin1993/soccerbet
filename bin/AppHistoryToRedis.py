...
def GiveSuggest(homeodd,awayodd,vec1,vec2):
    ...
    if homeodd<awayodd:
       if vec1[2]>w_Treshold and vec2[0]>w_Treshold:
          return 'h3' #home is up and home win
       if vec1[2]<l_Treshold and vec2[0]<l_Treshold:
          return 'h0' #home is up and home can't win
    if homeodd>awayodd:
       if vec2[2]>w_Treshold and vec1[0]>w_Treshold:
          return 'a3' #away is up and away win
       if vec2[2]<l_Treshold and vec1[0]<l_Treshold:
          return 'a0' #away is up and away can't win
    return '99'      
...
#-------------------   11.08 the task Tiwinly gave me ----------------------
...
def Task_GetMatch(match_id):
    ...
    if home_rv.num_matches<1 or away_rv.num_matches<1:
       return 0
    MySuggest = GiveSuggest(wodd,lodd,home_vec,away_vec) 
    WDL_IfRight = Task_WDLJudge(MySuggest,wodd,home_goal,away_goal)
    (BigBall,SmaBall)=BSc.CalGoal(home_rv.get_average_goal(),home_rv.get_average_lose(),away_rv.get_average_goal(),away_rv.get_average_lose())    
    BS_IfRight  = Task_BSJudge(BigBall,SmaBall,home_goal,away_goal)
    return (WDL_IfRight,BS_IfRight)

def Task_WDLJudge(MySuggest,wodd,home_goal,away_goal):
    if MySuggest == 'h3' and wodd>=1.7 and home_goal-away_goal>0:
       return 3
    elif MySuggest == 'h3' and wodd>=1.7 and home_goal-away_goal<0:
       return 0
    elif MySuggest == 'h0' and home_goal-away_goal<=0:
       return 3
    elif MySuggest == 'h0' and home_goal-away_goal>0:
       return 0      
    elif MySuggest == 'a3' and wodd<=4.0 and home_goal-away_goal<0:
       return 3
    elif MySuggest == 'a3' and wodd<=4.0 and home_goal-away_goal>0:
       return 0      
    elif MySuggest == 'a0' and home_goal-away_goal>=0:
       return 3      
    elif MySuggest == 'a0' and home_goal-away_goal<0:
       return 0      
    return 1

def Task_BSJudge(Bigball,Smaball,home_goal,away_goal):
    if Smaball<=home_goal+away_goal and Smaball>=2:
       return 3
    if Smaball>home_goal+away_goal and Smaball>=2:   
       return 0
    return 1      
...
def HD_GetMatch(match_id):
    ...
    (BigBall,SmaBall)=BSc.CalGoal(home_rv.get_average_goal(),home_rv.get_average_lose(),away_rv.get_average_goal(),away_rv.get_average_lose())    
    BS_IfRight  = Task_BSJudge(BigBall,SmaBall,home_goal,away_goal)
    return (home_rv.num_matches,away_rv.num_matches,home_vec,away_vec,BigBall,SmaBall,wodd,lodd,home_goal,away_goal)

if __name__ == '__main__':
    #WritePre(sys.argv[1])
    #test(698120)
    #m_match_ids = ['663190','663189','663191','663194']
    #print TestMultiMatches(4)
    #Task_Main(3)
    HD_main(5)
